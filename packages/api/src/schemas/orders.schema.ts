...
export const updateOrderStatusSchema = z
  ...
  .refine(
    (data) => {
      if (data.status === "CANCELLED" && !data.raison?.trim()) {
        return false;
      }
      return true;
    },
    { message: "La raison du refus est obligatoire", path: ["raison"] },
  );
...
export type OrderStatusValue = "PENDING" | "CONFIRMED" | "IN_DELIVERY" | "DELIVERED" | "CANCELLED";
